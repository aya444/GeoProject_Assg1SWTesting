...
import com.geo.geoproject.util.Preconditions;
...
public final class GeoHash {
    ...
    /**
     * The standard practical maximum length for .
     */
    ...
    /**
     * Returns the adjacent hash in given {@link Direction}.
    ...
    public static int hashLengthToCoverBoundingBox(double topLeftLat, double topLeftLon,
        ...
        for (int bits = 0; bits < MAX_HASH_LENGTH * 5; bits++) {
            if (isEven) {
                double mid = (minLon + maxLon) / 2; //45
                if (topLeftLon >= mid) { //35 false
                    if (bottomRightLon < mid) //20 
                        return bits / 5;
                    minLon = mid;
                } else {
                    if (bottomRightLon >= mid) //20 false
                        return bits / 5;
                    maxLon = mid; //maxlon=45
                }
            } else {
                double mid = (minLat + maxLat) / 2; //-45
                if (topLeftLat >= mid) { //35 true
                    if (bottomRightLat < mid) //false
                        return bits / 5;
                    minLat = mid; //minlat=-45
                } else {
